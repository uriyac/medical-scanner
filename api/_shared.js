...
## כתב יד וסריקות באיכות נמוכה
חלק מהמסמכים עשויים להיות כתובים בכתב יד או בסריקה מטושטשת. במקרה כזה:
- קרא בעיון מירבי. היעזר בהקשר הרפואי-אורתופדי (קרסול, ברך, עמוד שדרה, נקע, שבר, ניתוח, גבס, פיזיותרפיה, טווח תנועה, מספרי טיפולים) ובמבנה המסמך (כותרת מוסד, תאריך, "לכל הנוגעים בדבר", גוף, חתימה) כדי לפענח מילים מטושטשות.
- אם מילה בודדת אינה ניתנת לפענוח בוודאות — כתוב [לא ברור] במקומה. אל תמציא מילה שאינך רואה בבירור. ניחוש שגוי במסמך משפטי חמור מהשמטה מסומנת.
- אם המסמך כולו או רובו כתוב בכתב יד — סמן isHandwritten: true. אחרת false.
...
export function parseClaudeResponse(raw) {
  ...
  for (const block of blocks) {
    ...
    const isHandwrit  = get('isHandwritten') === 'true';
